import Barchat from '@/components/Admin-navigation/barchat';
import CardHeaders from '@/components/Admin-navigation/cards-headers'
import Linechat from '@/components/Admin-navigation/linechart';
import GeoMap from '@/components/Admin-navigation/map';
import { comunicadosDiarios, comunicadosMensuales, comunicadosPorTema } from '@/lib/user-action';
...
import PieChartComponent from '@/components/Admin-navigation/piechart';
import SupplierNeeds from '@/components/Admin-navigation/directorio';
import { redirect } from 'next/navigation';

export default async function AnaliticsScreen() {
  ...
  const ordersCount = await PressModel.countDocuments({ email: session.user.email }).sort({ _id: -1 })
  const ComunicadoresCount = await SliderModel.countDocuments()
  const usersCount = await User.countDocuments()
  const periodistasCount=await PeriodistaModel.countDocuments()
/* data table */
const orderDocs = await PressModel.find().sort({ _id: -1 });
const orders = JSON.parse(JSON.stringify(orderDocs));
//Graficos
const graphData3 = await comunicadosPorTema(); 
const graphData2 = await comunicadosDiarios(); 
const graphData = await comunicadosMensuales();
...
return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
			...
			<div className="bg-white rounded-[12px]">
			<Linechat data={graphData2} />
			</div>
			<div className="bg-white rounded-[12px]">
		<PieChartComponent data={graphData3}/>
					</div>
					<div className="bg-white rounded-[12px]">
